...
class LocalMusicRepository @Inject constructor(
    ...
    private fun saveData(str: String) {
        ...
        for (album in hotAlbumList) {
            for (i in 0 until album.getListAudio().size)
        ...
        for (effectAlbum in effectAlbumList) {
            for (i in 0 until effectAlbum.getListEffectAudio().size) {
                Timber.d("congnm effect album check downloaded ${effectAlbum.getListEffectAudio()[i]!!.name}")
